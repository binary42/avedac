...
public class InfiniteProgressPanel extends JComponent implements MouseListener {
    ...
    public void paintComponent(Graphics g) {
        if (started) {
            ...
            if ((text != null) && (text.length() > 0)) {
                Point2D.Float     pen     = new Point2D.Float((float) center.x, (float) maxY);
                FontRenderContext context = g2.getFontRenderContext();
                Font              myFont  = getFont().deriveFont(Font.BOLD);
                String[]          lines   = text.split("\n");

                g2.setColor(getForeground());

                for (int i = 0; i < lines.length; i++) {
                    TextLayout fragLayout = new TextLayout(lines[i], myFont, context);

                    pen.y += fragLayout.getAscent();
                    fragLayout.draw(g2, pen.x - (float) (fragLayout.getAdvance() / 2.0), pen.y);
                    pen.y += fragLayout.getDescent() + fragLayout.getLeading() + 3;
                }
            }
        }
    }
    ...
    private Area[] buildTicker() {
        ...
        for (double i = 0.0; i < (double) barsCount; i++) {
            ...
            AffineTransform toCircle  = AffineTransform.getRotateInstance(-i * fixedAngle, center.getX(),
                                            center.getY());
    ...
    protected class Animator implements Runnable {
        ...
        public void run() {
            ...
            AffineTransform toCircle       = AffineTransform.getRotateInstance(fixedIncrement, center.getX(),
                                                 center.getY());
