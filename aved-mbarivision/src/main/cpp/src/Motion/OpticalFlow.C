...
// ######################################################################
MbariOpticalFlow::MbariOpticalFlow
(std::vector<rutz::shared_ptr<MbariFlowVector> > flowVectors,
 Dims dims):
...
// ######################################################################
void MbariOpticalFlow::computeFlowLocations()
{
...
// ######################################################################
std::vector<Point2D<float> > MbariOpticalFlow::getFlowLocations()
{
...
// ######################################################################
std::vector<rutz::shared_ptr<MbariFlowVector> > MbariOpticalFlow::getFlowVectors()
{
...
// ######################################################################
Dims MbariOpticalFlow::getImageDims()
{
  return itsImageDims;
}

// ######################################################################
void MbariOpticalFlow::computeFlowVectors()
{
  LINFO("Computing flow vectors");
  ...
  while(dptr != stop)
    {
      ...
      if(fval > 0.0)
        {                   
          itsFlowVectors.push_back(rutz::shared_ptr<MbariFlowVector>
            (new MbariFlowVector
...
// ######################################################################
void MbariOpticalFlow::computeFlowField()
{
  LINFO("Computing flow fields");
  ...
  itsFlowField.first = dir;
  itsFlowField.second = len;
  itsFlowField.third = val;
...
// ######################################################################
Image<float> MbariOpticalFlow::getDirectionField()
{
  // compute the flow vectors from the flow field first
  if(!itsFlowFieldComputed)
    {
...
// ######################################################################
Image<float> MbariOpticalFlow::getVectorLengthField()
{
  // compute the flow vectors from the flow field first
  if(!itsFlowFieldComputed)
    {
...
// ######################################################################
Image<float> MbariOpticalFlow::getFlowStrengthField()
{
  // compute the flow vectors from the flow field first
  if(!itsFlowFieldComputed)
    {
