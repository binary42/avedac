...
// $HeadURL: svn://iLab.usc.edu/trunk/saliency/src/Image/FilterOps.C $
// $Id: FilterOps.C 10776 2008-06-05 06:25:27Z mundhenk $
//
...
#ifdef HAVE_OPENCV
  LDEBUG("Using OpenCV Templ Matching alg.");

#else
  LDEBUG("Using correlation alg.");
 ...
 */
  ...
  // counterclockwise:
  const int o0 =  dx, o1 =  dx - dy*w, o2 = -dy*w, o3 = -dx - dy*w,
            o4 = -dx, o5 = -dx + dy*w, o6 =  dy*w, o7 =  dx + dy*w;
...
namespace
{
  ...
  inline void JFILT(DstItr dptr, SrcItr sptr, int w, int jmax, int imax)
  {
...
GVX_TRACE(__PRETTY_FUNCTION__);
  ...
  const int o0 = dx, o1 = dx - dy*w, o2 = -dy*w, o3 = -dx - dy*w,
    o4 = -dx, o5 = -dx + dy*w, o6 = dy*w, o7 = dx + dy*w;
  typename Image<T>::iterator const rpp = result.beginw() + dx + dy*w;
  ...
  // the junction filter:
  {
    if (r[0]) JFILT(rpp, i0.begin()   + o0 + dx + dy*w, w, jmax, imax);
    if (r[1]) JFILT(rpp, i45.begin()  + o1 + dx + dy*w, w, jmax, imax);
    if (r[2]) JFILT(rpp, i90.begin()  + o2 + dx + dy*w, w, jmax, imax);
    if (r[3]) JFILT(rpp, i135.begin() + o3 + dx + dy*w, w, jmax, imax);
    if (r[4]) JFILT(rpp, i0.begin()   + o4 + dx + dy*w, w, jmax, imax);
    if (r[5]) JFILT(rpp, i45.begin()  + o5 + dx + dy*w, w, jmax, imax);
    if (r[6]) JFILT(rpp, i90.begin()  + o6 + dx + dy*w, w, jmax, imax);
    if (r[7]) JFILT(rpp, i135.begin() + o7 + dx + dy*w, w, jmax, imax);
  }
