...
Fern::Fern( const cv::Size& baseSize, unsigned int numTests, unsigned int numChannels )
...
void Fern::evaluate(Features& ft, const cv::Rect& ROI, cv::Mat& result, int stepSize, float threshold) const
{
	...
	for(int x = ROI.x; x < (ROI.x + ROI.width - m_baseSize.width); x+=stepSize)
	{
		...
		{
			it = m_nodeTable.find( calcIndex(ft, cv::Point(x,y)) );
...
int Fern::backProject(Features& ft, cv::Mat& projected, const cv::Rect& ROI, cv::Point& center, float radius, int stepSize, float threshold) const
{
	std::map< unsigned int, Node >::const_iterator it;
	std::map< unsigned int, Node >::const_iterator end = m_nodeTable.end();
...
void Fern::update(Features& ft, const cv::Point& pos, int label, const cv::Point& center)
{
	...
	{
		// insert new node
		Node newnode(MAP_SIZE, MAP_STEP);
		it = m_nodeTable.insert ( std::make_pair( idx, newnode) ).first;
	}

	Node& node = (*it).second;
	cv::Point vote = cv::Point(center.x-pos.x, center.y-pos.y);
