/*!@file VisualEvent.C classes useful for event tracking */
...
// $Id: MbariVisualEvent.C,v 1.6 2009/06/23 21:15:17 dcline Exp $
//
...
namespace MbariVisualEvent {
  ...
  { 
    ...
    return (getMaxSize() > itsDetectionParms.itsMinEventArea && \
            ...
            (int)  getNumberOfFrames() >=  itsDetectionParms.itsMinFramesEvent ? \
  ...
  void VisualEventSet::readHeaderFromStream(std::istream& is)
  {
    ...
    is >> itsDetectionParms.itsMinFramesEvent;
  ...
  void VisualEventSet::writeHeaderToStream(std::ostream& os)
  {
    ...
    os << itsDetectionParms.itsMaxDist << " "
       ...
       << itsDetectionParms.itsMinFramesEvent << " "
    ...
    while(currEvent != itsEvents.end()) {
      ...
      switch((*currEvent)->getState()) 
        ...
        case(VisualEvent::OPEN):
          if (itsDetectionParms.itsMaxFramesEvent > 0 && currFrame > ((*currEvent)->getStartFrame() + itsDetectionParms.itsMaxFramesEvent)){
            //limit event to itsMaxFrames
            LINFO("Event %i reached max frame count:%d - flagging as closed", (*currEvent)->getEventNum(),\
                  itsDetectionParms.itsMaxFramesEvent);
            (*currEvent)->close();		
          }
          break;
        default:
