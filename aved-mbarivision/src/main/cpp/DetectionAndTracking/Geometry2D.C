...
// $HeadURL: svn://iLab.usc.edu/trunk/saliency/src/MBARI/Geometry2D.C $
// $Id: Geometry2D.C,v 1.2 2009/04/16 23:58:06 dcline Exp $
//
...
// ######################################################################
Vector2D::Vector2D(const RealPoint2D& point)
  : itsX(point.i), itsY(point.j), valid(true)
{}
...
// ######################################################################
void Vector2D::reset(const Point2D& point)
{
  itsX = point.i;
  itsY = point.j;
  valid = true;
...
{
  ASSERT(isValid());
  return Point2D(int(itsX + 0.5F),int(itsY + 0.5F));
}
...
// ######################################################################
LineSegment2D::LineSegment2D(const Point2D& p1,
                             const Point2D& p2)
...
// ######################################################################
void LineSegment2D::reset(const Point2D& p1,
                          const Point2D& p2)
{
  itsPoint1 = p1;
  itsPoint2 = p2;
...
// ######################################################################
Point2D LineSegment2D::point2() const
{
  ASSERT(isValid());
  return (itsPoint2);
}
...
{
  float o = fabs((float)(point1().j - point2().j));
  float a = fabs((float)(point1().i - point2().i));

  return atan(o/a);
  //return atan((float)(point1().j - point2().j) / (float)(point1().i - point2.i()));
}
...
bool LineSegment2D::intersects(LineSegment2D &line, double &xcoord, double &ycoord) const
{
  ...
  if (!isFinite(x) || !isFinite(y) ||
      distance(RealPoint2D(x, y)) > 0.000010 ||
      line.distance(RealPoint2D(x, y)) > 0.000010  ) {
...
double LineSegment2D::distance(RealPoint2D point) const {
  Vector2D A(point1());
  Vector2D B(point2());
