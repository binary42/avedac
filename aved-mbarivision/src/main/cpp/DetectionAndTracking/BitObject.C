/*!@file BitObject.C class that holds an object defined by a shape in a
bit image */
...
// $Id: BitObject.C,v 1.1 2007/09/26 23:06:54 dcline Exp $
//
...
// ######################################################################
BitObject::BitObject(const Image<byte>& img, const Point2D location, 
...
// ######################################################################
Image<byte> BitObject::reset(const Image<byte>& img, const Point2D location, 
                             const byte threshold)
{
...
// ######################################################################
Point2D BitObject::getCentroid(const BitObject::Coords coords) const
{
          ...
          for (int dx = -1; dx <= 1; ++dx)
            {
              ...
              Point2D pim(x+dx,y+dy);
              if (!img.coordsOk(pim)) continue;

              bool isBoundary = false;
              Point2D pm(x+dx-l,y+dy-t);
