/*!@file Neuro/WinnerTakeAllStdOptimMbari.C Optimized version of WinnerTakeAllStd */
...
// $HeadURL: svn://iLab.usc.edu/trunk/saliency/src/Neuro/WinnerTakeAllStdOptimMbari.C $
// $Id: WinnerTakeAllStdOptimMbari.C 10299 2008-03-10 23:10:15Z farhan $
//

#ifndef NEURO_WinnerTakeAllStdOptimMbari_C_DEFINED
#define NEURO_WinnerTakeAllStdOptimMbari_C_DEFINED

#include "Neuro/WinnerTakeAllStdOptimMbari.H"

#include "Util/JobWithSemaphore.H"
#include "Util/MainJobServer.H"
#include "rutz/trace.h"

struct WinnerTakeAllStdOptimMbari::EvolveJob : public JobWithSemaphore
{
  ...
  { return "WinnerTakeAllStdOptimMbariEvolveJob"; }
...
// ######################################################################
// ######################################################################
// ########## WinnerTakeAllStdOptimMbari implementation
// ######################################################################
// ######################################################################

WinnerTakeAllStdOptimMbari::WinnerTakeAllStdOptimMbari(OptionManager& mgr,
                                             const std::string& descrName,
                                             const std::string& tagName) :
  WinnerTakeAllAdapter(mgr, descrName, tagName),
  itsTimeStep(SimTime::SECS(0.0000001)),
  itsEl(0.0F),
  itsEe(100.0e-3F),
  itsEi(-20.0e-3F),
  itsC(1.0E-9F),
...
// ######################################################################
WinnerTakeAllStdOptimMbari::~WinnerTakeAllStdOptimMbari()
{
GVX_TRACE(__PRETTY_FUNCTION__);
}

// ######################################################################
void WinnerTakeAllStdOptimMbari::reset1()
{
GVX_TRACE(__PRETTY_FUNCTION__);
  itsV.freeMem();
...
// ######################################################################
void WinnerTakeAllStdOptimMbari::input(const Image<float>& in)
{
GVX_TRACE(__PRETTY_FUNCTION__);
...
// ######################################################################
void WinnerTakeAllStdOptimMbari::doEvolve(const SimTime& t, Point2D<int>& winner)
{
GVX_TRACE(__PRETTY_FUNCTION__);
  winner.i = -1;
  ...
  const SimTime dt =
    SimTime::computeDeltaT(t - itsT, itsTimeStep);

  for (SimTime tt = itsT; tt < t; tt += dt)
    {
     LINFO("####### Debug t %f itsT %f ######", t.secs(), itsT.secs()); 
	if (tt == SimTime::ZERO())
        continue;
     LINFO("####### Debug 2 t %f itsT %f ######", t.secs(), itsT.secs()); 

      ASSERT(dt != SimTime::ZERO());

      const double dt_c = dt.secs() / itsC;
...
// ######################################################################
Image<float> WinnerTakeAllStdOptimMbari::getV() const
{
GVX_TRACE(__PRETTY_FUNCTION__);
  return itsV;
}
...
// ######################################################################
void WinnerTakeAllStdOptimMbari::inhibit()
{
GVX_TRACE(__PRETTY_FUNCTION__);
  itsGe = 0.0F;
  itsGi = itsGinh;
...
// ######################################################################
void WinnerTakeAllStdOptimMbari::saccadicSuppression(const bool on)
{
GVX_TRACE(__PRETTY_FUNCTION__);
...
// ######################################################################
void WinnerTakeAllStdOptimMbari::blinkSuppression(const bool on)
{
GVX_TRACE(__PRETTY_FUNCTION__);
...
#endif // NEURO_WinnerTakeAllStdOptimMbari_C_DEFINED
